...
array set BS {}
array set BL {}
foreach k {in_bits w_bits in_sign w_sign drain acc load other} { set B($k) 0.0 }
foreach k {in_bits w_bits in_sign w_sign drain acc load other} {
    set BI($k) 0.0
    set BS($k) 0.0
    set BL($k) 0.0
}
...
foreach_in_collection c $seq_cells {
    ...
    set p [expr {$pi + $ps + $pl}]
    if {[string match *in_bits_pipe_reg* $nm]} { set bucket in_bits
    } elseif {[string match *w_bits_pipe_reg* $nm]} { set bucket w_bits
    } elseif {[string match *in_signs_pipe_reg* $nm]} { set bucket in_sign
    } elseif {[string match *w_signs_pipe_reg* $nm]} { set bucket w_sign
    } elseif {[string match *drain_reg_reg* $nm]} { set bucket drain
    } elseif {[string match *acc_reg* $nm]} { set bucket acc
    } elseif {[string match *load_*pipe_q* $nm]} { set bucket load
...
set flops_total 0.0
foreach k {in_bits w_bits in_sign w_sign drain acc load other} { set flops_total [expr {$flops_total+$B($k)}] }
...
puts $fh "CSVROW,$DESIGN_NAME,[mw $B(in_bits)],[mw $B(w_bits)],[mw $B(in_sign)],[mw $B(w_sign)],[mw $B(acc)],[mw $B(drain)],[mw $B(load)],[mw $B(other)],[mw $popcount],[mw $clock_dist],[mw $glue],[mw $tot]"
close $fh
