...
module InnerPE #(
    ...
    // Operand registers are resetless. Stream bits advance every cycle; signs
    // update only when their registered load wave reaches this PE.
...
// The implementation uses dimensioned unpacked arrays. This adapter preserves
// the existing packed physical interfaces used by synthesis and gate benches.
module InnerPEFlat #(
    parameter int K = 8,
    parameter int M = 16,
    parameter int N_H = 9,
...
`define SC_MANUAL_PE_TOP(NAME, NP) \
module NAME ( \
    input logic clk, reset, mac_en, shift_in, \
    input logic [(NP)*6*16-1:0] a_bits_in, \
    input logic [(NP)*6-1:0] a_signs_in, \
    input logic [(NP)*6*16-1:0] w_bits_in, \
    input logic [(NP)*6-1:0] w_signs_in, \
    input logic load_a_sign_in, load_w_sign_in, \
    output logic [(NP)*6*16-1:0] a_bits_out, \
    output logic [(NP)*6-1:0] a_signs_out, \
    output logic [(NP)*6*16-1:0] w_bits_out, \
    output logic [(NP)*6-1:0] w_signs_out, \
    output logic load_a_sign_out, load_w_sign_out, \
    input logic [(NP)*24-1:0] acc_in_west, \
    output logic [(NP)*24-1:0] acc_out_east \
); \
    InnerPEFlat #( \
        .K(6), .M(16), .N_H(NP), .N_W(NP), .OWIDTH(24) \
    ) u_core (.*); \
endmodule

`SC_MANUAL_PE_TOP(sc_inner_pe_manual_k6m16n3_ow24, 3)
`SC_MANUAL_PE_TOP(sc_inner_pe_manual_k6m16n6_ow24, 6)
`SC_MANUAL_PE_TOP(sc_inner_pe_manual_k6m16n9_ow24, 9)

`undef SC_MANUAL_PE_TOP
