`ifndef PAYN_SIGNED_SEGMENTED_BLOCK_RETIRED_INNER_PE
`define PAYN_SIGNED_SEGMENTED_BLOCK_RETIRED_INNER_PE

`include "payn/variants/signed_segmented_block_retired/inner_tile_signed_segmented_block_retired.sv"

module InnerPESignedSegmentedBlockRetired #(
    parameter int K = 8,
    parameter int M = 16,
    parameter int T = 128,
    parameter int N_H = 9,
    ...
    localparam int PHASE_W =
        (BLOCK_CYCLES <= 1) ? 1 : $clog2(BLOCK_CYCLES);
    ...
    // The phase is common to every tile.  Counting only enabled MACs preserves
    // alignment across idle gaps, and shift resets the next accumulation to a
    // fresh block without adding a phase register to each tile.
    logic [PHASE_W-1:0] block_phase;
    logic block_last;
    ...
    assign block_last =
        (block_phase == PHASE_W'(BLOCK_CYCLES - 1));

    always_ff @(posedge clk) begin
        if (reset || shift_in) begin
            block_phase <= '0;
        end else if (mac_en) begin
            if (block_last)
                block_phase <= '0;
            else
                block_phase <= block_phase + PHASE_W'(1);
        end
    end
...
module InnerPESignedSegmentedBlockRetiredFlat #(
    parameter int K = 8,
    parameter int M = 16,
    parameter int T = 128,
    parameter int N_H = 9,
    ...
    for (genvar v = 0; v < N_W; v++) begin : g_w_ports
        for (genvar d = 0; d < K; d++) begin : g_depth
            ...
            assign w_signs_out[v*K + d] =
                w_signs_out_array[v][d];
        end
    end

    InnerPESignedSegmentedBlockRetired #(
        .K(K), .M(M), .T(T), .N_H(N_H), .N_W(N_W),
        .OWIDTH(OWIDTH)
    ) u_array_core (
        .clk,
        .reset,
