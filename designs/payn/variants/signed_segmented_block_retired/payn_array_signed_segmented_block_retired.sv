`ifndef PAYN_SIGNED_SEGMENTED_BLOCK_RETIRED_ARRAY
`define PAYN_SIGNED_SEGMENTED_BLOCK_RETIRED_ARRAY

`timescale 1ns/1ps

`include "payn/sobol.sv"
`include "payn/pe_peripheral.sv"
`include "payn/variants/signed_segmented_block_retired/inner_pe_signed_segmented_block_retired.sv"
...
`endif
`ifndef PAYN_BLOCK_T
`define PAYN_BLOCK_T 128
`endif

module payn_array_signed_segmented_block_retired #(
    parameter int K = `PAYN_K,
    parameter int M = `PAYN_M,
    parameter int T = `PAYN_BLOCK_T,
    ...
    InnerPESignedSegmentedBlockRetiredFlat #(
