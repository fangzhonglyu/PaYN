`ifndef PAYN_SIGNED_SEGMENTED_CLEAN_ARRAY
`define PAYN_SIGNED_SEGMENTED_CLEAN_ARRAY

`timescale 1ns/1ps

// Synthesis top for the cleaned pending-bit signed segmented PaYN array.
//
// Functionally identical to `payn_array_signed_segmented`, same ports and same
// parameters, so it drops into test_payn_array.sv / power_payn_array.sv via
// +define+PAYN_ARRAY_DUT.  The difference is confined to the tile: one shared
// HIGH_W adder serves both the pending retire and the visible high segment,
// replacing two ripple chains and two select banks.
...
// pending_carry / pending_borrow / *_pipe) are preserved so the existing PT
// breakdown and SAIF scripts key off this design unchanged.
...
module payn_array_signed_segmented_clean #(
