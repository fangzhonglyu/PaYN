`ifndef PAYN_WDBI_BITDECODE_INNER_PE
`define PAYN_WDBI_BITDECODE_INNER_PE

`include "payn/variants/wdbi/common/dbi_word_encode_next.sv"
`include "payn/variants/wdbi/bitdecode/inner_tile_wdbi_bitdecode.sv"

module InnerPEWDBIBitDecode #(
    parameter int K = 8,
    parameter int M = 16,
    parameter int N_H = 9,
    parameter int N_W = 9,
    ...
    // These are the long W transport roots.  They retain the previous encoded
    // word so the next cycle can minimize Hamming distance to what is already
    // present on the physical wires.
    ...
    // A data remains the native row broadcast.  Signs keep the baseline's
    // registered load-wave behavior.
    always_ff @(posedge clk) begin
    ...
    // Keep the wide data bank resetless.  A single resettable valid bit makes
    // the first post-reset word unencoded, avoiding a comparison against X.
    always_ff @(posedge clk) begin
        // This assignment is unconditional, matching the baseline bit pipe and
        // preventing reset from becoming a clock-enable on all M*N_W*K flops.
        for (int v = 0; v < N_W; v++)
            for (int d = 0; d < K; d++)
    ...
    for (genvar h = 0; h < N_H; h++) begin : g_row
        ...
        for (genvar v = 0; v < N_W; v++) begin : g_col
            InnerTileWDBIBitDecode #(
                .K(K), .M(M), .OWIDTH(OWIDTH)
            ) u_inner (
                .clk,
                .reset,
    ...
    for (genvar v = 0; v < N_W; v++) begin : g_w_output
        for (genvar d = 0; d < K; d++) begin : g_depth
            // Preserve the public InnerPE contract: systolic outputs are raw,
            // even though this single-PE experiment transports W encoded.
...
module InnerPEWDBIBitDecodeFlat #(
    parameter int K = 8,
    parameter int M = 16,
    parameter int N_H = 9,
    parameter int N_W = 9,
