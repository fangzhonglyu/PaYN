`ifndef PAYN_SIGNED_SEGMENTED_CSA_INNER_PE
`define PAYN_SIGNED_SEGMENTED_CSA_INNER_PE

`include "payn/variants/signed_segmented_csa/inner_tile_signed_segmented_csa.sv"

module InnerPESignedSegmentedCSA #(
    parameter int K = 8,
    parameter int M = 16,
    parameter int N_H = 9,
    parameter int N_W = 9,
    ...
    initial begin
        assert (K == 8)
            else $error("signed_segmented_csa currently requires K=8");
    ...
    for (genvar h = 0; h < N_H; h++) begin : g_row
        ...
        for (genvar v = 0; v < N_W; v++) begin : g_col
            ...
            InnerTileSignedSegmentedCSA #(
                ...
                .acc_out_valid(acc_out_valid_nc)
            );
        end
    end
...
module InnerPESignedSegmentedCSAFlat #(
    parameter int K = 8,
    parameter int M = 16,
    parameter int N_H = 9,
    parameter int N_W = 9,
