`ifndef PAYN_SIGNED_SEGMENTED_INNER_PE_GRID
`define PAYN_SIGNED_SEGMENTED_INNER_PE_GRID

`include "payn/variants/signed_segmented/inner_pe_signed_segmented.sv"

// Two-dimensional outer grid of signed-segmented InnerPEs.
//
...
// each global accumulator row drains east through every PE column.
//
...
module InnerPESignedSegmentedGrid #(
    ...
    initial begin
        assert (P_ROWS > 0) else $error("P_ROWS must be positive");
        assert (P_COLS > 0) else $error("P_COLS must be positive");
    end
    ...
    for (genvar r = 0; r < P_ROWS; r++) begin : g_a_edges
        ...
        for (genvar h = 0; h < N_H; h++) begin : g_row
            ...
            for (genvar d = 0; d < K; d++) begin : g_depth
                ...
                assign a_bits_out[r][h][d] =
                    a_bits_link[r][P_COLS][h][d];
                assign a_signs_out[r][h][d] =
                    a_signs_link[r][P_COLS][h][d];
            end
        end
    end
    ...
    for (genvar c = 0; c < P_COLS; c++) begin : g_w_edges
        ...
        for (genvar v = 0; v < N_W; v++) begin : g_col
            for (genvar d = 0; d < K; d++) begin : g_depth
                ...
                assign w_bits_out[c][v][d] =
                    w_bits_link[c][P_ROWS][v][d];
                assign w_signs_out[c][v][d] =
                    w_signs_link[c][P_ROWS][v][d];
            end
        end
    end
