`ifndef PAYN_SIGNED_SEGMENTED_INNER_TILE
`define PAYN_SIGNED_SEGMENTED_INNER_TILE
...
// A carry/borrow across the 2**LOW_W boundary is encoded as a one-cycle pending
// event and retired into the upper accumulator on the following clock.  The
// visible output includes that pending event, so it is canonical at all times
// after combinational settling and preserves the baseline shift behavior.
module InnerTileSignedSegmented #(
    parameter int K = 8,
    parameter int M = 16,
    ...
    // Include the pipelined boundary event in the externally visible high
    // segment.  This makes acc_out canonical even before the next clock folds
    // the event into acc_high.
