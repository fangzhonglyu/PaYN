`ifndef PAYN_SIGNED_SEGMENTED_CENTERED_INNER_TILE
`define PAYN_SIGNED_SEGMENTED_CENTERED_INNER_TILE

// Exact centered-residue direct-retire signed segmented accumulator for one
// PaYN inner tile.
//
...
// the same edge as the residue. The stored word is biased by 2**(LOW_W-1), so
// numerical zero is centered in the low digit instead of sitting on a radix
// boundary. The enclosing PE converts only the row boundaries; tile-to-tile
// drain traffic remains in this encoded representation.
module InnerTileSignedSegmentedCentered #(
    parameter int K = 8,
    parameter int M = 16,
    ...
    localparam int CENTER_BIAS = 1 << (LOW_W - 1);
